...
from jsngram.users import serializers as user_serializers
from jsngram.notifications import views as notification_views

class Images(APIView):
    
    def get(self, request, format=None):

        user = request.user # 접속한 유저의 ID

        following_users = user.following.all() # 팔로잉한 유저들 리스트

        image_list = []

        for following_user in following_users:

            user_images = following_user.images.all()[:2] # 팔로잉한 각 유저들의 이미지를 2개씩 가져온다. 
